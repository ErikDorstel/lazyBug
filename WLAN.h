...
void WiFiStationConnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  preferences.begin("lazyBug",false); preferences.putString("ssidStation",ssidStation); preferences.putString("passwordStation",passwordStation); preferences.end();
...
void WiFiStationDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  preferences.begin("lazyBug",false); String ssidStationOld=preferences.getString("ssidStation",""); String passwordStationOld=preferences.getString("passwordStation",""); preferences.end();
  if (ssidStationOld==ssidStation & passwordStationOld==passwordStation & WiFi.softAPgetStationNum()==0) { WiFi.reconnect(); } }

void initWLAN() {
  preferences.begin("lazyBug",false); ssidStation=preferences.getString("ssidStation",""); passwordStation=preferences.getString("passwordStation",""); preferences.end();
  ...
  WiFi.setHostname("lazyBug"); WiFi.begin(ssidStation.c_str(),passwordStation.c_str());
